...
export class ReponseConsultationComponent implements OnInit {
  ...
  constructor(private reponseConsultationService: ReponseConsultationService, private route: ActivatedRoute) {
    this.route.params.subscribe(parameters => {
      this.reponseConsultationService.findByIdPrestataire(645).subscribe(resp => {this.infoPresta = resp; console.log(resp);
      }, error => console.log(error));
      this.reponseConsultationService.findByIdMOE(636).subscribe(resp => {this.infoMOE = resp; console.log(resp);
      }, error => console.log(error));
    })
  }
