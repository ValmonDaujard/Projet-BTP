...
export class ProjetMOService {
  ...
  modifyActionWithoutType(action: Action) {
  ...
  createAction(action: Action) {
    return this.http.post<Action>("http://localhost:8080/action", action);
  }

  deleteReunionById(id: number) {
    return this.http.delete("http://localhost:8080/reunion/" + id);

  }

  createReunion(reunion: Reunion) {
    return this.http.post<Reunion>("http://localhost:8080/reunion", reunion);
  }
}
